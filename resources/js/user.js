...
import { AjaxGetData } from "./setupAjax";
...
$(document).ready(function () {
    ...
    $('#btnCreate').click(function () {
        ...
        showModal("modalUser", "Add User", "btnCreateform", "<i class='fas fa-save'></i> Simpan");
    });
    ...
    $(document).on('click', '.btnEdit', function () {
        showModal("modalUser", "Edit User", "btnEditform", "<i class='fas fa-regular fa-pen'></i> Update");
        clearErrorMsg();
        ...
        $.ajax({
            ...
            dataType: "json",
            success: function (response) {
                getKondisiData(response.data[0].role, response.data.nim ?? response.data.nip)
    ...
    $(document).on('click', '#btnEditform', function () {
        ...
        $('#nip').val() ? ($('#nip').attr('disabled') != "disabled" ? formData.append('nip', $('#nip').val()) : '') : ''; // hanya mengirim variabel role jika role tidak ada atribut disabled
        $('#nim').val() ? ($('#nim').attr('disabled') != "disabled" ? formData.append('nim', $('#nim').val()) : '') : ''; // hanya mengirim variabel role jika role tidak ada atribut disabled
